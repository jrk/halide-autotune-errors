...
int main(int argc, char **argv) {
    ...
    final(x, y, c) = normalize(x, y, c);
    {
        ...
        .split(y, y, _y1, 4)
        .split(c, c, _c2, 2)
        .reorder(_x0, x, _y1, _c2, y, c)
        .reorder_storage(x, y, c)
        .vectorize(_x0, 4)
        .compute_at(Halide::Func(funcs["downsampled"]), _c5)
        ;
        Halide::Func(funcs["downsampled"])
        .split(x, x, _x3, 8)
        .split(y, y, _y4, 8)
        .split(c, c, _c5, 2)
        .reorder(_c5, _x3, c, x, _y4, y)
        ...
        .split(x, x, _x6, 16)
        .split(c, c, _c8, 4)
        .reorder(_x6, y, _c8, x, c)
        .reorder_storage(y, x, c)
        .vectorize(_x6, 8)
        .compute_at(Halide::Func(funcs["interpolated$2"]), y)
        ;
        Halide::Func(funcs["downx$2"])
        .reorder(c, y, x)
        .reorder_storage(y, x, c)
        .compute_at(Halide::Func(funcs["interpolated$2"]), _x12)
        ;
        Halide::Func(funcs["interpolated$2"])
        .split(x, x, _x12, 2)
        .split(y, y, _y13, 32)
        .reorder(_y13, y, _x12, c, x)
        .reorder_storage(c, y, x)
        .vectorize(_y13, 4)
        .parallel(x)
        .compute_root()
