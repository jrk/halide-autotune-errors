...
Halide::Var _x0, _y1, _x2, _y3, _c4, _x5, _y6, _z7, _c8, _x9, _y10, _z11, _x12, _y13, _z14, _x15, _y16, _z17, _x18, _y21;
clamped .split(x, x, _x0, 4) .split(y, y, _y1, 4)
.reorder(_x0, _y1, x, y) .reorder_storage(y, x)
.vectorize(_x0, 2) .parallel(y)
.compute_root()
;
histogram .split(x, x, _x2, 8) .split(y, y, _y3, 4) .split(c, c, _c4, 64)
.reorder(_c4, _x2, c, x, _y3, y) .reorder_storage(x, y, c)
.vectorize(_c4, 16)
.compute_at(blury, _y13)
;
grid .split(x, x, _x5, 8) .split(y, y, _y6, 16) .split(z, z, _z7, 2) .split(c, c, _c8, 16)
.reorder(_c8, _y6, c, _x5, _z7, y, x, z) .reorder_storage(x, z, c, y)
.vectorize(_c8, 8)
.compute_at(blury, _z14)
;
blurx .split(x, x, _x9, 4) .split(y, y, _y10, 32) .split(z, z, _z11, 4)
.reorder(_y10, _z11, _x9, y, z, x) .reorder_storage(x, y, z)
.vectorize(_y10, 8)
.compute_at(blury, _x12)
;
blury .split(x, x, _x12, 2) .split(y, y, _y13, 8) .split(z, z, _z14, 4)
.reorder(_x12, _z14, _y13, x, y, z) .reorder_storage(y, z, x)
.parallel(z)
.compute_root()
;
blurz .split(x, x, _x15, 4) .split(y, y, _y16, 8) .split(z, z, _z17, 2)
.reorder(_y16, _z17, _x15, y, z, x) .reorder_storage(y, z, x)

.compute_root()
;
interpolated .split(x, x, _x18, 8)
.reorder(_x18, y, x) .reorder_storage(y, x)
.vectorize(_x18, 2)
.compute_at(bilateral_grid, _y21)
;
bilateral_grid .split(y, y, _y21, 16)
.reorder(_y21, x, y) .reorder_storage(y, x)
