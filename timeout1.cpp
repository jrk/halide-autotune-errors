...
Halide::Var _x0, _y1, _x2, _y3, _x5, _y6, _c8, _x9, _z11, _x12, _y13, _y16, _x20, _y21;
clamped .split(x, x, _x0, 16) .split(y, y, _y1, 2)
.reorder(_x0, _y1, y, x) .reorder_storage(y, x)
.unroll(_x0, 4) .vectorize(_x0, 2)
.compute_root()
;
histogram .split(x, x, _x2, 1024) .split(y, y, _y3, 4)
.reorder(_x2, x, c, _y3, y) .reorder_storage(y, c, x)
.unroll(_x2, 256) .vectorize(_x2, 16)
.compute_at(blury, x)
;
grid .split(x, x, _x5, 8) .split(y, y, _y6, 1024) .split(c, c, _c8, 8)
.reorder(_y6, y, _x5, _c8, z, x, c) .reorder_storage(x, z, c, y)
.unroll(_y6, 64) .vectorize(_y6, 16)
.compute_at(blurx, _x9)
;
blurx .split(x, x, _x9, 16) .split(z, z, _z11, 32)
.reorder(_z11, _x9, x, y, z) .reorder_storage(y, x, z)
.unroll(_z11, 8) .vectorize(_z11, 4)
.compute_at(blury, _x12)
;
blury .split(x, x, _x12, 32) .split(y, y, _y13, 16)
.reorder(_x12, x, _y13, y, z) .reorder_storage(z, x, y)
.vectorize(_x12, 16)
.compute_root()
;
blurz .split(y, y, _y16, 256)
.reorder(_y16, y, x, z) .reorder_storage(y, x, z)
.unroll(_y16, 32) .vectorize(_y16, 4)
.compute_at(bilateral_grid, y)
;
interpolated
.reorder(x, y) .reorder_storage(y, x)
.unroll(x, 32) .vectorize(x, 2)
.compute_at(bilateral_grid, _y21)
;
bilateral_grid .split(x, x, _x20, 8) .split(y, y, _y21, 64)
.reorder(_y21, y, _x20, x) .reorder_storage(y, x)
.unroll(_y21, 16) .vectorize(_y21, 4) .parallel(x)
.compute_root()
;
