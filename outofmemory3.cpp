...
Halide::Var _x0, _x2, _y3, _c4, _x5, _y6, _z7, _c8, _x9, _y10, _z11, _x12, _y13, _y16, _z17, _x18, _y19, _x20, _y21;
clamped .split(x, x, _x0, 4)
.reorder(y, _x0, x) .reorder_storage(x, y)
.vectorize(y, 4) .parallel(x)
.compute_root()
;
histogram .split(x, x, _x2, 64) .split(y, y, _y3, 2) .split(c, c, _c4, 8)
.reorder(_x2, _c4, c, _y3, y, x) .reorder_storage(y, x, c)
.vectorize(_x2, 8)
.compute_at(grid, c)
;
grid .split(x, x, _x5, 8) .split(y, y, _y6, 16) .split(z, z, _z7, 16) .split(c, c, _c8, 4)
.reorder(_c8, c, _x5, _z7, _y6, x, y, z) .reorder_storage(y, x, c, z)

.compute_root()
;
blurx .split(x, x, _x9, 4) .split(y, y, _y10, 4) .split(z, z, _z11, 32)
.reorder(_z11, _x9, x, _y10, z, y) .reorder_storage(z, y, x)
.vectorize(_z11, 2)
.compute_at(blury, _y13)
;
blury .split(x, x, _x12, 2) .split(y, y, _y13, 4)
.reorder(_y13, _x12, y, x, z) .reorder_storage(y, x, z)
.vectorize(_y13, 2)
.compute_at(blurz, _z17)
;
blurz .split(y, y, _y16, 4) .split(z, z, _z17, 8)
.reorder(x, _y16, _z17, y, z) .reorder_storage(x, z, y)
.vectorize(x, 8) .parallel(z)
.compute_root()
;
interpolated .split(x, x, _x18, 32) .split(y, y, _y19, 2)
